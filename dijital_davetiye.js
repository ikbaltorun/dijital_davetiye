...
const timer = setInterval(function () {
  ...
  if (distance < 0) {
    clearInterval(timer);
    document.getElementById("countdown").innerHTML = "<h3>Büyük Gün Geldi!</h3>";
  }
}, 1000);
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (photoStack) {
        photoStack.addEventListener('click', function(event) {
            ...
            if (clickedItem.classList.contains('is-flying') || clickedItem.classList.contains('is-hidden')) return;
    ...
    if (rsvpForm) {
        rsvpForm.addEventListener('submit', (e) => {
            ...
            let statusText = attendance === 'katiliyorum' ? "Evet, Katılacağım ✨" : "Hayır, Katılamayacağım ❤️";
            ...
            if (attendance === 'katiliyorum') {
                formMessage.innerHTML = `<h3>Teşkezőler ✨</h3><p>Sayın <strong>${name}</strong>, katılım durumunuz (Evet, Katılacağım) başarıyla kaydedildi ve WhatsApp'a yönlendiriliyorsunuz...</p>`;
            } else {
